...
By default it targets `http://localhost:5114` (the API's default HTTP launch URL). You can override it with:

```bash
VITE_API_BASE_URL=http://localhost:5114 npm run dev
```
...
If your browser blocks API requests, make sure backend CORS is enabled and running on the same URL.
